...
if __name__ == '__main__':
    df_for_sentiment = pd.read_csv('../Hackathon_eleven/Modelling/Models/df_for_sentiment.csv', index_col=0)
    # We load the model
    ...
    df_for_sentiment['review_text'] = df_for_sentiment['review_text'].map(lambda x: ''.join(c for c in x if c == '_' or c not in string.punctuation).split())
    ...
    dict3_grams_filtered = gensim.corpora.Dictionary.load('../Hackathon_eleven/Modelling/Models/model_saved.model.id2word')
    ...
    df_topics = format_topics_sentences(ldamodel=LDA_model, corpus=corpus3grams, documents=docs3grams)
    df_topics_tokens = df_topics.reset_index()
    # We clean and add some infos on the tokens
    df_topics_tokens.columns = ['index', 'main_topic', 'percent_contribution_of_topic', 'keywords', 'tokens', 'text']
    df_topics_tokens['number_tokens'] = df_topics_tokens['tokens'].map(lambda x: len(x) if hasattr(x, '__len__') else 0)
    df_topics_tokens['unique_tokens'] = df_topics_tokens['tokens'].map(lambda x: list(set(x)) if hasattr(x, '__iter__') else [])
